...
Tables = {
	init: function() {
		...
		if ($('.js-datatable-orders').length) {
			$('.js-datatable-orders').DataTable({
				'oLanguage': {
					'oPaginate': {
						'sPrevious': '<i class="fa fa-angle-left"></i>',
						'sNext': '<i class="fa fa-angle-right"></i>'
...
Forms = {
	init: function() {
		var charcounter = $('.js-char-counter');
		if (charcounter.length) {
			charcounter.charCount({
				allowed: charcounter.data('char-allowed'),
				warning: charcounter.data('char-warning'),
				css: 'counter',
		...
		if ($('.js-max-length').length) {
			$('.js-max-length').maxlength();
		}

		if ($('.js-pwstrength').length) {
			$('.js-pwstrength').pwstrength({
				showVerdicts: false
			});
		}

		if ($('.js-colorpicker-hex').length) {
			$('.js-colorpicker-hex').colorpicker({
				format: 'hex'
			});
		}
		if ($('.js-colorpicker-rgb').length) {
			$('.js-colorpicker-rgb').colorpicker({
				format: 'rgb'
			});
		}

		if ($('.js-colorpicker-box').length) {
			$('.js-colorpicker-box').colorpicker();
		}

		if ($('.js-mention').length) {
			$('.js-mention').mention({
				users: [{
					username: 'john',
					name: 'John Jones',
		...
		if ($('#wysiwyg2').length) {
			var editor = new wysihtml5.Editor('wysiwyg2', {
				toolbar: 'toolbar',
